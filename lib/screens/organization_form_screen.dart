...
class _OrganizationFormScreenState extends State<OrganizationFormScreen> {
  ...
  Future<void> _pickImage() async {
    if (kIsWeb) {
      ...
      setState(() => _pickedFile = PlatformFile(name: file.name, size: file.size, bytes: bytes));
    } else {
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty || desc.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please fill name and description')));
      return;
    }
    ...
    try {
      ...
      await coll.doc(docId).set({
        ...
        if (widget.initialData?['banner'] != null) 'banner': widget.initialData!['banner'],
      }, SetOptions(merge: true));

      if (_pickedFile != null) {
        final ref = FirebaseStorage.instance.ref('organizations/$docId/${_pickedFile!.name}');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(isEdit ? 'Edit Organization' : 'New Organization', style: const TextStyle(color: Colors.white)),
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(
                        _pickedFile?.name ?? _initialBannerUrl?.split('/').last ?? 'No banner selected',
                      ),
                    ),
                    TextButton(onPressed: _pickImage, child: const Text('Pick Banner')),
                  ],
                ),
                if (_loading) ...[
                ...
                ElevatedButton(
                  ...
                  child: Text(isEdit ? 'Save Changes' : 'Create Organization'),
                ),
              ],
            ),
          ),
